...
const SYSTEM_PROMPT: Message = {
  ...
  content: 'You are a helpful assistant.\n' +
    ...
    'If you write a mathematical expression, use Unicode instead of LaTeX.'
};

const MAX_LENGTH = Number(process.env.NEXT_PUBLIC_MAX_INPUT_LENGTH) || 4000;

const ReactMarkdownContainer = React.memo(({ msg }: { msg: Message }) => (
      ...
      <div className='flex-1 overflow-y-auto pt-4 px-4'>
        {messages
          ...
          .map((msg, index) => (
            ...
            >
              <div
                className={`rounded-lg px-4 py-2 max-w-full md:max-w-xl
                  ${msg.role === 'user'
                    ? 'bg-blue-600 text-white'
                    : 'bg-gray-700 text-gray-100'
                  }`}
              >
                {msg.content.startsWith('data:image') && msg.role !== 'user' ? (
                  <img
                    src={msg.content}
                  ...
                  <div className='whitespace-pre-wrap wrap-anywhere'>
                    {msg.role === 'assistant' ? (
                      <ReactMarkdownContainer msg={msg} />
                    ) : (
                      <>
                        {msg.content.trim()}
                      </>
